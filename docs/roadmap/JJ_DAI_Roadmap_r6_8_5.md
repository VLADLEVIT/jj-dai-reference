# JJ DAI · Роадмэп до запуска сети · r6.8.5

**JAI · DECENTRALIZED ARTIFICIAL INTELLIGENCE**

Единый документ · включает r6.7 целиком + шестой сквозной трек «Отложенная верификация и оффлайн-режим узла» + Agent Alpha как цель фазы · 25 августа 2026 · jj-dai.org
...
| **Онтология Alpha** *(r6.8.1)* | «узел, на котором хранитель проводит цикл» | четыре термина ADR-020 rev 4.1, A1: **Alpha Harness** (клонируется) · **Alpha Profile(k)** · **существо на харнессе** (не клонируется) · **Alpha Host Conformance**. Первое существо — **`JaiGuruDev`** |
...
**Текущий билд — v0.6.8.** Статус называется одной формулой, чтобы документ не противоречил сам себе: **`code-complete · 215/215 recorded · untagged · релизный долг открыт`** (206/206 — первый кат; доработка добавила девять проверок: пять в r6.8.3, две по аудиту recut1, две по аудиту recut2). Проверено из чистой распаковки; тег не ставится и публикация не производится, потому что не закрыта цепочка поставки — подписанные артефакты и two-person release approval, — и нет T-TOOLSET. **CLA сюда не входит:** она регулирует приём чужого вклада и на тег не влияет (см. «Разграничение долгов»). Галочка «✓» в таблице дропов означала бы закрытие, а по собственному правилу этого документа дроп есть **тег** с зелёным acceptance — тега нет. v0.6.6 закрытым не считается: его исправления поглощены v0.6.7.
...
| **v0.6.10** | **T-TOOLSET вместе с остатком supply chain: стартовый набор wasm, воспроизводимая сборка, подписанные артефакты, two-person release approval, подпись манифеста, витнессирование добавления инструмента.** SBOM, пиннинг инструментария и `.gitattributes` выполнены раньше — в доработке v0.6.8 (r6.8.3). Здесь же закрывается production-рука: до этого дропа обязательный живой путь Alpha заканчивается pure-ответом | план |
...
**Четвёртое окно — v0.6.9, ADR-020 rev 4.1** (введено в r6.8.1, приведено к принятой ревизии в r6.8.3). Перечень воспроизводит §3 ADR-020 rev 4.1 дословно; расхождение между этим списком и §3 ADR читается в пользу ADR:
...
**Согласованный набор изменений ADR-020 — четыре части, ни одна не действует отдельно.** Резерв есть одна из них; резка v0.6.9 без остальных трёх запрещена:

1. **ADR-020 rev 4.1** — сам документ, Accepted;
...
Требование к дереву перед резкой v0.6.9: ADR-020 rev 4.1, поправка ADR-017 / A-1 и настоящая ревизия роадмэпа лежат в `docs/`, покрыты `tree_digest` и названы в индексе ADR.
...
**Четыре термина, и смешение любых двух — источник ошибок** (ADR-020 rev 4.1, A1). r6.8 писал «узел, на котором хранитель проводит цикл», и это смешивало все четыре сразу:
...
* **build backend** — только по точной версии. Хэша здесь нет и быть не может: PEP 517 не даёт `[build-system].requires` поля под него. Это единственный вход, разрешаемый по имени во время сборки, и он **назван открытым долгом** в v0.6.10 и объявлен в SBOM как `build-backend-unhashed`.

Остальное содержание потока — воспроизводимая сборка, подписанные артефакты, release provenance и two-person approval — остаётся открытым и вынесено в v0.6.10. Инвентарь — не доказательство происхождения: SBOM говорит, что лежит в коробке, и ничего не говорит о том, кто её запечатал.
...
* релизный долг закрыт: канонический AGPL байт-в-байт (**выполнено**, v0.6.7), SBOM, пиннинг python-зависимостей по хэшу и экшенов по коммиту (**выполнено**, доработка v0.6.8), воспроизводимая сборка вместе с хэшированием build backend, подписанные артефакты, release provenance и two-person release approval — после чего v0.6.x получает первый тег. **CLA в этот перечень не входит:** она регулирует приём чужого вклада и тег не блокирует (см. «Разграничение долгов»).
...
**Новые в r6.8.1 (из ADR-020 rev 4.1 и ADR-021 rev 4):**
...
Что он **не делает** и что обязано стоять в его аннотации: не является релизом; не утверждает ничего о происхождении — **SBOM присутствует как инвентарь**, но сборка не воспроизводима, артефакты не подписаны и two-person approval не выполнен; **не расходует номер версии**, `v0.6.8` остаётся доступным для тега релиза.
...
**Текущий билд: v0.6.8 — `code-complete · 215/215 recorded · untagged · релизный долг открыт`.** Проверено из чистой распаковки, drift чист. Трек v0.6.x / фаза Ф0 — в работе.
...
* **релизный долг — всё, что перечислено, открыто:** воспроизводимая сборка, **build backend по хэшу** (PEP 517 не даёт поля — требует другого механизма), подписанные артефакты, release provenance, two-person release approval, T-TOOLSET, остаток ре-ката 3 (README вне `tree_digest`; режимы файлов и цели симлинков). **Проверено по дереву: модули wasm отсутствуют — `deploy/wasm-toolset/toolset.json` несёт механизм с пустым `tools`; CLA отсутствует, но тег не блокирует;**
* **Audit 0** с включёнными в объём ADR-014…ADR-021. **ADR-020 принята как rev 4.1**, поэтому условие «принять до резки v0.6.9» выполнено; на его место встаёт условие о дереве — до резки v0.6.9 в `docs/` должны лежать ADR-020 rev 4.1, поправка ADR-017 / A-1, ADR-021 и настоящая ревизия роадмэпа, а `tree_digest` должен быть перезаписан прогоном по этому дереву. ADR-021 rev 4 остаётся Proposed и критический путь не держит;
...
*r6.8.5 · единый документ: включает r6.7 целиком (фазы, гейты, нумерация билдов, Backend & Model Portability, Contest & Containment, канарейки, когнитивная непрерывность, Читта, тулсет wasm, GUI хранителя), сквозной трек «Отложенная верификация и оффлайн-режим узла» и Agent Alpha как цель фазы Ф0 · шестнадцать правок по ревью ADR-020 внесены в блоки A-ALPHA, G-GUI, таблицу дропов, окна резерва и критерии гейтов Ф0–Ф2; r6.8.2 добавляет предполётные теги, называет перенос невыполненного из v0.6.7, вносит байтовую точность дерева в supply chain и обновляет статусы ADR; r6.8.3 приводит резерв ADR-020 к принятой rev 4.1, называет согласованный набор изменений, унифицирует статус v0.6.8, снимает CLA с причин отсутствия тега, исправляет путь записанного прогона, добавляет четвёртое условие предполётного тега и переносит SBOM, пиннинг и `.gitattributes` в доработку v0.6.8; r6.8.4 приводит четыре блока о текущем состоянии в соответствие с деревом после доработки, переписывает релизный долг перечнем действительно открытого и фиксирует ADR-020 как Accepted; r6.8.5 переписывает операционную норму `.gitattributes`, называет границы пиннинга по трём входам сборки и вводит проверяемую свежесть PDF роадмэпа · чтение r6.7, r6.8 и r6.8.1 не требуется · подготовлено по кодовой базе v0.6.8, Architecture Map, CHANGELOG, ASIC RFI/RFP v2.2, Whitepaper 1.5, Хартии v1.1, ADR-014 (с поправкой A-1), ADR-015, ADR-016 rev 2, ADR-017, ADR-018 rev 2, ADR-019 rev 3.1, ADR-020 rev 4.1 (Accepted, в дереве), поправке ADR-017 / A-1 (встроена как K4-bis) и ADR-021 rev 4 (Proposed) · JJ GROUP · 25 августа 2026*
