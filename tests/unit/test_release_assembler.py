...
def test_the_context_hands_readers_not_hashes():
    ...
    # around.
    # THE REFUSAL IS PROVOKED, NOT ASSUMED. The first cut of this check
    # asserted that the ambient host could not build a context — true in
    # the container it was written in (no git history, network answering)
    # and false on the auditor's host (a git checkout, network refusing), so
    # the check failed there for no defect of the tree. A check that depends
    # on where it runs measures the room, not the code. So the network probe
    # is stubbed to ANSWER, and the D3 gate must refuse on exactly that.
    # The git epoch is stubbed too, so that the ONE remaining reason to
    # refuse is the network: otherwise a host without a git history refuses
    # on the committer time and the assertion below is satisfied by the
    # wrong gate — the same room-measuring, one door over.
    real_probe, real_epoch = asm.be.network_is_unreachable, \
        asm.be.source_date_epoch
    asm.be.network_is_unreachable = lambda *a, **k: False
    asm.be.source_date_epoch = lambda *a, **k: "1700000000"
    try:
        try:
            asm.context("v0.6.9")
            raise AssertionError("a context was built while the network "
                                 "answered the probe")
        except AssertionError:
            raise
        except Exception as exc:                               # noqa: BLE001
            assert "network is reachable" in str(exc), exc
    finally:
        asm.be.network_is_unreachable = real_probe
        asm.be.source_date_epoch = real_epoch
