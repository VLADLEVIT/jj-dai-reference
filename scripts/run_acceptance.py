...
def source_digest(root: str = None, *, source: str = None) -> str:
    ...
    """
    digest, _source, _count = _tree_digest(ROOT if root is None else root, source)
    return digest
...
def _tree_digest(root: str, source: str = None):
    ...
    #
    # `source=None` lets D11 choose: the COMMITTED tree inside a git
    # checkout, the bytes on disk anywhere else. That is right for the
    # recorded run — `hermetic.json` must say `git-committed-tree` from a
    # clean clone — and wrong for a probe that edits the working copy and
    # expects the digest to notice: the first git clone of this tree showed
    # two such probes green in an unpack and red in the clone, because the
    # committed tree does not see an uncommitted edit. A probe of what is ON
    # DISK names the worktree source explicitly.
    if source is not None:
        return _source_tree.digest(root, source=source, require_clean=False)
    chosen = _source_tree.digest(root, require_clean=False)
    if chosen[1] != _source_tree.SOURCE_GIT:
        return chosen
    # ATTRIBUTION. Inside a checkout D11 addresses the commit — but the
    # tests just ran against the bytes on disk, and if those differ from
    # the commit, a record saying `git-committed-tree` would attribute a run
    # to sources that did not execute. The audit of recut9 named the risk
    # exactly: after editing the working copy, the helper still answered
    # the old digest under the committed label. So the committed label is
    # EARNED by equality — the worktree digest must equal the committed
    # one — and otherwise the run is recorded as what it was: a worktree.
    executed = _source_tree.digest(root, source=_source_tree.SOURCE_WORKTREE,
                                   require_clean=False)
    if executed[0] != chosen[0]:
        return executed
    return chosen
