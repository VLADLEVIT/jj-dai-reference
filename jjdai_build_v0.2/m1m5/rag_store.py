...
class RagStore:
    ...
    # ---------- merkle (domain-separated, second-preimage safe) ----------
    # Leaf and node digests carry distinct domain prefixes, and we hash raw
    # bytes (not hex-string concatenation). This makes it impossible to re-
    # present an internal node as a leaf — the classic Merkle second-preimage
    # attack that plain sha256(a+b) admits. This RE-BASELINES all namespace
    # roots vs the pre-v2 plain scheme (intended; proofs carry scheme="domain-v2").
    _LEAF = b"\x00"
    _NODE = b"\x01"
    _EMPTY = sha256(b"\x02")          # domain-tagged empty tree
    ...
    @staticmethod
    def _leaf(chunk_hash: str) -> str:
        return sha256(RagStore._LEAF + bytes.fromhex(chunk_hash))

    @staticmethod
    def _parent(a: str, b: str) -> str:
        return sha256(RagStore._NODE + bytes.fromhex(a) + bytes.fromhex(b))

    def merkle_root(self, ns: str) -> str:
        level = [self._leaf(h) for _, h in self._leaves(ns)]
        if not level:
            return RagStore._EMPTY
        while len(level) > 1:
            if len(level) % 2:
    ...
    def prove(self, ns: str, chunk_id: int) -> dict | None:
        ...
        if idx is None:
            return None
        chunk_hash = leaves[idx][1]
        level = [self._leaf(h) for _, h in leaves]
        path = []
        while len(level) > 1:
            if len(level) % 2:
                level.append(level[-1])
            ...
            level = [self._parent(level[i], level[i + 1])
                     for i in range(0, len(level), 2)]
            idx //= 2
        return {"chunk_id": chunk_id, "leaf": chunk_hash,
                "path": path, "root": level[0], "scheme": "domain-v2"}

    @staticmethod
    def verify_proof(proof: dict) -> bool:
        h = RagStore._leaf(proof["leaf"])     # chunk hash -> domain-separated leaf
